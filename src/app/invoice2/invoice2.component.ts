...
export class Invoice2Component {
  ...
  totalSum: number = 0;
  myFormValueChanges$!: Observable<any>;
  ...
  getControls() {
    return (this.exampleForm.get('units') as FormArray).controls;
  }
  ...
  private updateTotalUnitPrice(units: any) {
    ...
    for (let i in units) {
      ...
      let totalUnitPriceFormatted = totalUnitPrice
   ...
   */
  private getCountryByIpOnline(): void {
